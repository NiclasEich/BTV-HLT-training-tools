...
def main():
    ...
    parser.add_argument("--file1", "-i1", help="Input 1 root-file, this should be mc", type=str)
    ...
    with u3.open(path_mc) as file_mc, u3.open(path_data) as file_data:

        tree_mc = file_mc["ttree"]
        tree_data = file_data["btagana/ttree"]

        mask_mc = tree_mc["Jet_pt"].array() > 25.
        ...
        for key_mc, key_data in key_pairs.items():
            ...
            try:
                arr_mc = tree_mc[key_mc].array()[mask_mc]
            except ValueError as e:
                arr_mc = tree_mc[key_mc].array()[ak.any(mask_mc, axis=-1)]
            ...
            mc = ak.to_numpy( ak.flatten(arr_mc, axis=-1))
        ...
        for key in new_ntuple_keys:
            if key in keys_data:
                ...
                try:
                    arr_mc = tree_mc[key].array()[mask_mc]
                except ValueError as e:
                    arr_mc = tree_mc[key].array()[ak.any(mask_mc, axis=-1)]
                except IndexError as e:
                    arr_mc = tree_mc[key].array()[ak.any(mask_mc, axis=-1)]
