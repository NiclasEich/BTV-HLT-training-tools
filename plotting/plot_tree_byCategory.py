...
for vertexCategory in [-999.0, 0.0, 1.0, 2.0]:
    plot_dir = os.path.join( base_dir, "vertex_cat_{0:1d}".format(int(vertexCategory)) )
    ...
    if args.isData is True:
        ...
        online_mask = online_tree["TagVarCSV_vertexCategory"].array() == vertexCategory 

    online_mask = on_pt_mask & online_mask
    print("Category:\t{}".format(vertexCategory))
    ...
    for key  in keys: 
        ...
        if key in list(map(lambda x: x.decode("utf-8"), online_tree.keys())):
            ...
            plot_histogram(online_data, key, process_name, vertexCategory)